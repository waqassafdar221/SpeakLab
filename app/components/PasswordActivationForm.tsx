...
import { userApi, TokenManager, LoginResponse, InviteInfo } from '@/lib/api';
...
export interface PasswordActivationFormProps {
  getInfo: (token: string) => Promise<InviteInfo>;
  submit: (token: string, password: string) => Promise<LoginResponse>;
  headline: string;
  linkMissingMessage: string;
  linkInvalidMessage: string;
  activatingLabel: (username: string) => string;
  fallbackLabel: string;
  submitLabel: string;
  successMessage: string;
}

export default function PasswordActivationForm({
  getInfo,
  submit,
  headline,
  linkMissingMessage,
  ...
  useEffect(() => {
    if (!token) {
      setLinkError(linkMissingMessage);
      setIsChecking(false);
      return;
    }
    getInfo(token)
      .then((data) => setInfo(data))
      .catch((err) => setLinkError(err instanceof Error ? err.message : linkInvalidMessage))
      .finally(() => setIsChecking(false));
  }, [token, getInfo, linkMissingMessage, linkInvalidMessage]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await submit(token, password);
      ...
      <Card sx={cardSx}>
        <Box sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center', mb: 4 }}>
          ...
          <Typography variant="body2" sx={{ color: '#4a4a4a', textAlign: 'center' }}>
            {info ? activatingLabel(info.username) : fallbackLabel}
          </Typography>
        </Box>
        ...
        <Box component="form" onSubmit={handleSubmit}>
          ...
          >
            {isSubmitting ? <CircularProgress size={24} sx={{ color: '#fff' }} /> : submitLabel}
          </Button>
        </Box>
      </Card>
