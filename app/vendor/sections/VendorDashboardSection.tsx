...
export default function VendorDashboardSection() {
  const [stats, setStats] = useState<AdminStats | null>(null);
  const [customersGrowth, setCustomersGrowth] = useState<UsersGrowthPoint[]>([]);
    ...
    const load = async () => {
      try {
        const [statsData, growthData, usageData] = await Promise.all([
          vendorApi.getStats(),
          vendorApi.getCustomersGrowth(),
          vendorApi.getJobsUsage(),
        ]);
        setStats(statsData);
        setCustomersGrowth(growthData.map((p) => ({ ...p, date: formatDateLabel(p.date) })));
          ...
          <Card sx={chartCardSx}>
            ...
            <Box sx={{ mt: 2, height: 220 }}>
              <ResponsiveContainer width="100%" height="100%">
                <LineChart data={customersGrowth} margin={{ top: 4, right: 8, left: -20, bottom: 0 }}>
                  ...
                  <Tooltip
                    ...
                    itemStyle={{ color: '#fff' }}
                    formatter={(val) => [val ?? 0, 'New customers']}
                  />
                  <Line
                    type="monotone"
                    dataKey="count"
